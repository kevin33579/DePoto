...
class CraniImageServices {
  ...
  String generateFolderPath(String prefix, String numbers) {
    final now = DateTime.now();
    final formattedDateFolder = DateFormat('yyyy/MM/dd/').format(now);
    return '/DKM/OUT/$formattedDateFolder/$prefix$numbers';
  }
  ...
  Future<List<String>> renameFolderImage(String oldPrefix, String oldNumbers,
      String newPrefix, String newNumbers, bool isDMG, String date) async {
    final folderPath = 'DKM/OUT/$date/';
    print(date);
    ...
    for (final file in files) {
      if (file.startsWith('$oldPrefix$oldNumbers')) {
        var newFileName;
        if (isDMG == true) {
          newFileName = file.replaceFirst(
              '$oldPrefix${oldNumbers}DMG', '$newPrefix${newNumbers}DMG');
        } else {
          newFileName = file.replaceFirst(
              '$oldPrefix${oldNumbers}', '$newPrefix${newNumbers}');
        }
  ...
  Future<List<String>> renameFolderImageOnIsDmg(
      String prefix, String numbers, bool isDMG, String date) async {
    final folderPath = 'DKM/OUT/$date/';
    print(date);
    var folderPathOld;
    if (isDMG == true) {
    ...
    for (final file in files) {
      if (file.startsWith('$prefix$numbers')) {
        ...
        if (isDMG == true) {
          newFileName =
              file.replaceFirst('$prefix${numbers}', '$prefix${numbers}DMG');
        } else {
          newFileName =
              file.replaceFirst('$prefix${numbers}DMG', '$prefix${numbers}');
        }
        String oldFilePath = '$folderPathOld/$file';
        String newFilePath;
        if (isDMG == true) {
